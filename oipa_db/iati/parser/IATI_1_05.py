...
from oipa_db.iati import models
from oipa_db.iati.parser.exceptions import FieldValidationError, RequiredFieldError
from oipa_db.iati.parser.IATI_2_01 import Parse as IATI_201_Parser
from oipa_db.iati_codelists import models as codelist_models
from oipa_db.iati_organisation import models as organisation_models
...
class Parse(IATI_201_Parser):
    ...
    def iati_activities__iati_activity__reporting_org(self, element):
        ...
        super(
            Parse, self
        ).iati_activities__iati_activity__reporting_org(element)

        activity_reporting_organisation = self.get_model(
        ...
        if element.text:
            ...
            if activity_reporting_organisation.organisation:
                activity_reporting_organisation.organisation.primary_name = self.get_primary_name(  # NOQA: E501
                    element,
                    activity_reporting_organisation.organisation.primary_name
                )

                try:
                    if activity_reporting_organisation.organisation.name\
                            .narratives.count() == 0:
                        self.add_narrative(
                            element,
                    ...
                    organisation_name\
                        .organisation = activity_reporting_organisation\
                        .organisation
    ...
    def iati_activities__iati_activity__participating_org(self, element):
        ...
        role = self.get_or_none(
            codelist_models.OrganisationRole, name=role_name)
        ...
        super(
            Parse, self
        ).iati_activities__iati_activity__participating_org(element)
    ...
    def iati_activities__iati_activity__activity_date(self, element):
        ...
        super(
            Parse, self
        ).iati_activities__iati_activity__activity_date(element)
    ...
    tag:name"""
        super(
            Parse, self
        ).iati_activities__iati_activity__location__name(element)
    ...
    tag:description"""
        super(
            Parse, self
        ).iati_activities__iati_activity__location__description(
            element
        )
    ...
    tag:sector"""
        vocabulary = self.sector_vocabulary_mapping.get(
    ...
    def iati_activities__iati_activity__country_budget_items__budget_item__description(  # NOQA: E501
        ...
        super(
            Parse,
            self
        ).iati_activities__iati_activity__country_budget_items__budget_item__description(element)  # NOQA: E501
    ...
    def iati_activities__iati_activity__humanitarian_scope(self, element):
        ...
        super(
            Parse, self
        ).iati_activities__iati_activity__humanitarian_scope(element)
    ...
    tag:policy-marker"""
        vocabulary = self.policy_marker_vocabulary_mapping.get(
            element.attrib.get('vocabulary'))
        ...
        super(
            Parse, self
        ).iati_activities__iati_activity__policy_marker(element)
    ...
    def iati_activities__iati_activity__document_link__title(self, element):
        ...
        super(
            Parse, self
        ).iati_activities__iati_activity__document_link__title(element)
    ...
    def check_narrative_duplicate(self, parent, child, fk_field):
        """
        Given a 1.05 narrative mimicking element, like all results free text
        elements, make sure only one parent object is created, and add a
        narrative to that object
        """
    ...
    def iati_activities__iati_activity__result__description(self, element):
        ...
        # if a narrative for this result's result_description has already been
        # added
        if self.check_narrative_duplicate(
                result, result_description, 'result'):
    ...
    def iati_activities__iati_activity__result__indicator__title(
        ...
        if self.check_narrative_duplicate(
                result_indicator, result_indicator_title, 'result_indicator'):
        ...
        if element.text:
            self.add_narrative(element, result_indicator_title)
            result_indicator_title.primary_name = self.get_primary_name(
                element, result_indicator_title.primary_name)
    ...
    def iati_activities__iati_activity__result__indicator__description(
        ...
        result_indicator = self.get_model('ResultIndicator')
        result_indicator_description = self.get_model(
            'ResultIndicatorDescription')

        if self.check_narrative_duplicate(
                result_indicator, result_indicator_description,
                'result_indicator'):
    ...
    def iati_activities__iati_activity__result__indicator__baseline__comment(
        ...
        result_indicator = self.get_model('ResultIndicator')
        result_indicator_baseline_comment = self.get_model(
    ...
    def iati_activities__iati_activity__result__indicator__period__target__comment(self, element):  # NOQA: E501
        ...
        result_indicator_period_target_comment = self.get_model(
    ...
    def iati_activities__iati_activity__result__indicator__period__actual__comment(self, element):  # NOQA: E501
        result_indicator_period = self.get_model('ResultIndicatorPeriod')
        result_indicator_period_actual_comment = self.get_model(
